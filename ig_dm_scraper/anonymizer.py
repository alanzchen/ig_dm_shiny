...
def _anonymize_person_name(text: str, sender_name_set: set):
    ...
    if not pattern_found:
        # Second, find names using Name Entity Recognition approach with NLTK
        tree = ne_chunk(pos_tag(word_tokenize(text)))
        named_entities = [" ".join(w for w, t in entity) for entity in tree if isinstance(entity, nltk.Tree) and entity.label() == 'PERSON']
        anon_text = text
        for entity in named_entities:
            anon_text = anon_text.replace(entity, "[person]")

        # Third, find names using names from sender names approach
